							...
							THEN
								'(
									SELECT TOP(@i)
										...
										' +
										CASE
											WHEN OBJECT_ID('[master].[SQLAdmin].fn_varbintohexstr') IS NOT NULL THEN
												'[master].[SQLAdmin].fn_varbintohexstr(x.sql_handle) AS sql_handle,
												[master].[SQLAdmin].fn_varbintohexstr(x.plan_handle) AS plan_handle,'
											ELSE
